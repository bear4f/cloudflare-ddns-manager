...
Telegram 只会在 DNS 记录创建或 IP 变化更新成功后推送。安装 Bot 命令服务后，自动 DDNS 更新成功会在聊天底部生成一个新的按钮控制面板；如果 Bot 面板不可用，才会退回普通文字通知。
...
安装完成或重启 Bot 服务后，Telegram 输入框左侧的“菜单”会显示快捷命令。也可以直接发送 `/start` 或 `/panel` 打开带图按钮控制面板。

按钮操作会尽量在同一条面板消息里动态刷新进度和结果，不再把后台日志整段刷到 Telegram。详细执行记录仍保存在服务器日志 `/var/log/cf_ddns.log`。如果面板图片发送失败，日志里会记录 Telegram 返回的错误，并自动退回文字面板。
